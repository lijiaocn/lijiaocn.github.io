--- 
layout: default
title: "Envoy Proxy使用介绍（二）：envoy源代码阅读、集成开发环境(IDE)"
author: 李佶澳
createdate: "2018-12-17 16:06:24 +0800"
changedate: "2018-12-17 16:06:24 +0800"
categories: 项目
tags: envoy 视频教程
...
IDE建议使用`vscode`，见[在Mac上准备Envoy的开发环境](https://www.lijiaocn.com/2018/12/17/envoy-source-code.html#%E5%9C%A8mac%E4%B8%8A%E5%87%86%E5%A4%87envoy%E7%9A%84%E5%BC%80%E5%8F%91%E7%8E%AF%E5%A2%83)，之前用Clion折腾了很久，发现Clion不能正确解读[用bazel-cmakelists生成CMakeLists.txt](https://www.lijiaocn.com/2018/12/17/envoy-source-code.html#%E7%94%A8bazel-cmakelists%E7%94%9F%E6%88%90cmakeliststxt)。

~~使用C/C++开发使用的Clion阅读envoy代码，Clion使用cmake管理构建项目的，envoy使用的是bazel。~~

~~Clion中是可以安装一个名为bazel的插件的，不知为何安装之后没有效果，和网上的教程也不匹配。~~
...
include目录只需要包含`include`和`source`目录就可以了，因为envoy的代码在引用头文件的时候，使用是相对于inlucde和和source的路径，例如：
...
如果要在Clion中编译envoy，并且希望查看到envoy依赖的代码，可以尝试一下Github上一个名为[bazel-cmakelists](https://github.com/lizan/bazel-cmakelists)的工具，
这个工具可以为使用Bazel的C++ 项目生成CMakeLists.txt。

本地需要安装有bazel，mac可用brew[安装bazel](https://docs.bazel.build/versions/master/install-os-x.html#install-on-mac-os-x-homebrew)：
...
最后得到`CMakeLists.txt`以后，需要修正其中的路径，因为这个文件是在容器生成的，文件中的路径是容器中的路径，如果与容器外路径不一致，需要修改为容器外的路径，例如：
...
```
在Mac上还需要设置docker，在docker的`Perferences->File Sharing`面板中添加刚创建的`/build`目录，这一步一定要有，否则会报下面的错误：
...
在docker配置面板中添加/build目录之后，需要点击配置面板中的`Apply&Start`才能生效。
...
生成CMakeLists.txt之前，会调用bazel编译envoy，envoy第一次编译的时间非常非常长，4核CPU的情况下都要要几个小时。
...
出现这个问题的原因不明确，有时候这一次运行和下一次运行错误不同，怀疑是bazel在运行时要用git拉取很多代码，但是网速速度比较慢，有些代码拉取失败，导致后续一些列错误。
将/build目录中文件清除后，在网络状况好的时候重试就可以了。
...
还经常遇到下面的问题`Server crashed during startup`，原因不明，重新执行就好了，似乎某部外部条件导致bazel crash：
...
研究了bazel半天，没找到添加预定义宏的地方，最后直接把代码修改了：
...
编译完成，在envoy目录中生成了`CMakeLists.txt`文件之后，直接将envoy目录导入到vscode中，就可以愉快的读代码、写代码了。
...
[Find all references](https://github.com/Microsoft/vscode-cpptools/issues/15)中讨论很久，没耐心看他们在讨论些什么..

尝试了一下讨论中提到的[cquery](https://marketplace.visualstudio.com/items?itemName=cquery-project.cquery)插件，安装后发现可用，over。

还需要[安装cquery](https://github.com/cquery-project/cquery/wiki/Building-cquery)，在mac上可以直接用brew安装：

	brew install cquery

但是cquery和vscode官方的c++插件冲突，后来发现[C++ Intellisense](https://docs.microsoft.com/en-us/visualstudio/ide/visual-cpp-intellisense?view=vs-2017)更好一些。
