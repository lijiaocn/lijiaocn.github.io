---
layout: default
title: "Envoy（一）：新型L3~L7层访问代理软件Envoy的使用"
author: 李佶澳
createdate: "2018-12-12 20:13:08 +0800"
changedate: "2018-12-12 20:13:08 +0800"
categories:  项目
tags: envoy 视频教程
keywords: envoy,L7 proxy,请求代理
description: 一个较新的7层代理软件，专门为现在的面向服务架构设计的，据说已经在Lyft、apple、google等公司应用
---
...
Envoy的设计思路是“再增加一层”，对应用Server屏蔽了依赖服务的地址，使应用Server看到的一个单一、纯粹的网络环境（只需要与localhost交互）。站在每个应用Server角度上看，应用只需要与localhost交互，不需要知道依赖的其它服务的地址，通过访问loaclhost地址就能实际访问到远端依赖的服务，envoy默默地进行了来回地代理。

Envoy是伴随应用Server部署的，每个应用Server都有一个对应的Envoy，Envoy接管了这个应用所有的流量，负责将应用的流量转发到目的地址。

“加层”的设计思路，使Envoy没有“侵入性”，从而不受应用使用的编程语言的限制，适用范围更广。“加层”还使应用Server看到的网络环境纯粹而统一，从而使应用的部署更加一致、方便。

Envoy使用C++11开发，兼顾了性能和开发效率。

Envoy首先是一个L3/L4代理，然后在此基础上通过filter插件的方式实现了L7代理，以及对HTTP/2、gRPC、MongoDB L7、DynamoDB L7等协议的支持。负载均衡、健康检查、统计、限速等常见需求都支持。

## 工作过程

用一句话就可以交代清楚： 

`Host`上的`Downstream`通过本地envoy提供的多个`Listener`中的一个，连接由多个`Upstream`组成的`Cluster`，多个这样的Host组合在一起就是一个`Mesh`。

Envoy是单进程多线程的实现，master线程负责管理、接收连接请求，连接被接受后交给worker进程，100%无阻塞。

Listener可以创建多个，建议在每台机器上部署一个envoy，创建多个listner满足不同需求，这样便于管理和统计，Listner现在只支持TCP。

每个Listener都是单独配置各自的L3/L4层filter插件。
...
Envoy不提供已经编译好的二进制的文件，需要自己[编译](https://www.envoyproxy.io/docs/envoy/latest/install/building#building)，编译过程很繁琐，放在后面介绍。
...
Envoy用到了最新的C++语法，需要用5以及以上版本的gcc编译，这一点需要特别注意，CentOS默认安装的还是gcc 4.8。[gcc的编译](https://gcc.gnu.org/releases.html](https://gcc.gnu.org/releases.html)比较复杂。
建议还是用envoy提供的docker镜像编译，编译环境还可以保持一致，这里就使用这种方式。
...
默认的编译镜像的系统是ubuntu，用它编译得到的envoy依赖glibc-2.8，在使用glibc-2.7的centos7上运行是会遇到动态链接库不匹配的问题：
...
这个问题没有找到好的解决方法，后面章节记录当时的排查过程，后来发现envoy还有一个基于centos的编译镜像：
...
用这个镜像编译得到的envoy，在centos上运行是没有依赖库不匹配的问题，通过环境变量设置要使用的编译镜像：
...
变量`ENVOY_DOCKER_BUILD_DIR`的值是存放编译生成的文件的路径。

### 自己在容器外准备编译环境

用envory提供的镜像编译，只需要一行命令：

	./ci/run_envoy_docker.sh './ci/do_ci.sh bazel.dev'

需要对这个命令背后发生的事情有所了解，搞清楚了编译过程，才能知道从哪里开始阅读代码，以及怎样修改或添加、删除新的代码文件。

脚本`./ci/run_envoy_docker.sh`的用途是启动编译容器，将参数传入到容器内执行，主要就是脚本的最后一行：
...
将当前目录挂载到了容器的source目录：`-v "$PWD":/source`，然后在source目录中执行传入的参数`cd source && $*`。

关键操作在`./ci/do_cli.sh`中进行，以`bazel.dev`目标为例：
...
`source/exe/main.cc`就是程序的入口。

编译得到的文件在哪里？`do_cli.sh`中的命令将其拷贝到`${ENVOY_DELIVERY_DIR}`目录：
...
可以看到`${ENVOY_DOCKER_BUILD_DIR}`就是编译得到文件的存放路径，默认是`/tmp/envoy-docker-build`，这在里面可以找到编译得到的文件。
...
如果上面命令出错，进入buildifier的代码目录中，用bazel编译
...
配置文件是yaml格式的，envoy项目提供了一个配置文件样例子[google_com_proxy.v2.yaml](https://github.com/envoyproxy/envoy/blob/master/configs/google_com_proxy.v2.yaml)：
...
配置文件有`admin`、`static_resources`和`clusters`三部分组成。
...
测试环境与[API网关Kong学习笔记（十九）：Kong的性能测试](https://www.lijiaocn.com/%E9%A1%B9%E7%9B%AE/2018/12/03/kong-features-19-kong-performance.html#%E7%8E%AF%E5%A2%83)中的环境一致。

在部署kong的机器上部署了envoy，配置文件envoy.yaml内容如下：
...
测试方法也与[API网关Kong学习笔记（十九）：Kong的性能测试](https://www.lijiaocn.com/%E9%A1%B9%E7%9B%AE/2018/12/03/kong-features-19-kong-performance.html#%E7%8E%AF%E5%A2%83)中一致，upstream服务也相同。
...
单纯转发方面，Envoy和优化后的Kong相差不多，低于预期，Envoy的转发性能应当与Nginx不相上下，需要查询下资料，看看是否存在配置不当或理解有误的地方。

## 代理性能继续测试

在Google Group中找到Matt Klein对应Envoy的性能测试的[解释](https://groups.google.com/forum/#!topic/envoy-users/Tgd3HZMfIiU)。

默认情况下，envoy会开启一个比较影响性能的特性，比如request ID generation和dynamic stat generation。
将开启了这些特性的Envoy和Nginx进行对比是不合理的。
