...
ab建立了长连接以后，haproxy频繁reload，预期会有较多的haproxy进程会因为连接还存在，而一直没有退出：
...
因为曾遇到haproxy的配置文件中已经配置了监听地址，但是访问失败的情况。
...
将ab暂停，然后重新建立长链接，期待新建的长链接会被最新的haproxy进程处理：

	tcp 0 523 10.39.0.140:80 10.4.110.62:59719 ESTABLISHED 1684/haproxy

出现了预期外的情况，发现新建的长链接被1684号进程处理，而非最新运行的haproxy。
再次重建连接，又被另一个旧的haproxy进程处理:
...
多个haproxy监听了同一个端口是最直接的原因，在[连接haproxy间歇性失败的问题调查][1]中，
阅读[haproxy的文档][2]时，曾经遇到说明:
...
也就从kernel 3.9版本开始，监听端口是可以复用的，haproxy使用了这个特性。这个特性使新的haproxy进程
可以快速创建，不需要考虑旧的haproxy的是否释放了端口。

在lwn.net中找到了对`SO_REUSEPORT`的详细说明：[The SO_REUSEPORT socket option][3]

	Incoming connections and datagrams are distributed to the server sockets using a hash 
	based on the 4-tuple of the connection—that is,:
	the peer IP address and port plus the local IP address and port.
...
这个连接里的内容说明了一切：[Truly Seamless Reloads with HAProxy – No More Hacks!][4]
...
猜测还有其它的问题，根据在前面的操作中遇到的一些情况，进行了第二次实验。
...
断开长链接后，预期对是6244进程会因为已经没有连接存在了而退出，但是发现6244进程依然存在：

	6954 haproxy    0:00 haproxy -f /etc/haproxy/haproxy.cfg -db -sf 6938
	7090 haproxy    0:00 haproxy -f /etc/haproxy/haproxy.cfg -db -sf 7086

在当前环境下，再建立一个长链接，这时候有一个haproxy进程上有连接:

	ab -n 100000 -c 1 -k http://webshell-lijiaob-space.odev.enncloud.cn/

手动做haproxy reload，在-sf后面指定当前存在所有的haproxy进程号：
...
可以看到原先的两个haproxy进程只剩下了一个。

断开长连接后，这是存在的两个haproxy上都没有了连接，这时候手动reload:
...
这个进程没有退出。需要重新对它发送一个`SIGUSR1`信号。

## 解决方法

针对本文中复现的现象，修改了reload过程。原先reload的时候`-sf`只会指定上一个haproxy进程的
进程号。将其改为`-sf 系统中所有haproxy的进程号`以后，问题消失:
