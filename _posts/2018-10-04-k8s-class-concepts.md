...
[上一节][1]中，成功用minikube启动了一个本地kubernetes（可惜是1.10的，1.12的没成功），用kubeadm部署则[尴尬的失败了](https://www.lijiaocn.com/%E9%A1%B9%E7%9B%AE/2018/10/03/k8s-class-deploy.html#%E9%97%AE%E9%A2%981-kubeadmi-init%E5%A4%B1%E8%B4%A5kube-apiserver%E4%B8%8D%E5%81%9C%E9%87%8D%E5%90%AF)。
...
我这里是在做教程，就“择新不择旧”了，哪个版本新用哪个，可以避免严重脱节。

前面的minikube和kubeadm部署1.12版本都没有成功，本来应该先讲一下怎样全手动的把kubernetes1.12部署起来，定制每一个组件、敲定每一个参数。但是，这个过程会有一些繁琐，甚至有一些波折。

先了解一些kubernetes中的一些基本概念，或许更好一些，不然后面容易感觉云里雾里。

这一节，你只要知道这些概念就可以了，以后遇到的时候感觉面熟就行。

我的习惯还是借花献佛，下面的内容是对[Kubernetes Concepts][2]的二次加工。
...
注意，这里的“支持”是说，Kubernetes可以将这些类型的存储挂载到容器中，而不是说它提供了这些存储系统。计算、存储、网络，基础设施的三大块，Kubernetes只做计算这块。
