...
changedate: 2018/08/30 15:29:44
categories: 技巧
tags: prometheus
...
动态地为Prometheus添加监控目标（目标IP）和告警规则。

采用的方案是监控目标写入到[consul][3]中，以service的形态存放于consul中，prometheus通过服务发现，自动从consul发现监控目标。

告警规则是用key-value的方式记录在consul中，同时在prometheus端使用[confd][2]，confd监测consul中的数据，自动更新配置文件并触发加载。

consul的使用见：[服务治理工具consul的功能介绍与使用入门][3]。

confd的使用见：[配置文件动态生成工具confd的使用][2]。

## 配置prometheus，动态发现监控目标

首先确定要在consul中写入的监控目标的格式，约定格式如下：
...
prometheus的告警规则文件可以是独立的yml文件，用confd监控consul中的规则，动态更新它。
...
同样调用consul的api，写入到consul中，注意这里是kv的方式：
...
注意reload_cmd，同时prometheus重新加载规则文件。
...
在Promethues的配置文件引入confd生成的最终文件：
