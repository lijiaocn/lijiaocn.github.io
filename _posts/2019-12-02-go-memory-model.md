...
last_modified_at: "2019-12-02 17:40:27 +0800"
categories: 编程
cover:
tags: golang
...
这篇笔记不是对 [The Go Memory Model][2] 的直接翻译，是核心思想的提炼，因此更易懂。
但有一个问题，文章中提到的错误案例的错误结果很难复现，因此我们只能鉴定这篇文章是最权威的，它所说的都是对的，并按照它的指示去做。

## 宗旨

宗旨就一句话：如果多个协程并发操作同一块内存，要用并且只能用 Go 提供的 sync、sync/atomic 等方法控制内存的读写顺序，不要凭感觉自创方法。
