...
# OpenWrt
创建时间: 2014/04/25 11:59:24  修改时间: 2014/05/17 11:59:04 作者:lijiao
...
openwrt的代码库中没有提供所有的代码，而是在编译时会从指定的位置下载源码，所以需要处于联网状态.
...
启动一个vmware虚拟机,增加一块新的虚拟硬盘用来安装openwrt.

> 新的硬盘类型选择IDE, 否则openwrt启动时会找不到对应驱动，从而无法加载文件系统

使用dd(linux)或者physdiskwrite(windows)，将下载的img写到新增加的硬盘上

设置虚拟机从新增加的硬盘上启动, 直接进入系统 .  http://blog.wifizoo.net/?post=230

### 系统配置

http://wiki.openwrt.org/doc/start#configuring.openwrt
