...
现有的小程序代码能够升级为多端应用（微信开发者工具->工具->升级为多端项目），按照提示进行操作即可，升级时会提示到微信开发者平台上创建一个和当前小程序绑定的多端应用。

升级成功后可以进行以下操作：

1. 在微信开发者工具右上角进行小程序模式和多端应用模式之间切换，
2. 多端模式下，右上角选择 android/ios 模拟器以及真机运行（复用 xcode 以及 android studio 的模拟器）

如果在模拟器上或者真机上运行失败，可能是以下原因：

1. 当选择本地模拟器时，mac 上的微信开发者工具默认构建的可能是 x86 架构的 app（没找到解决方法）
2. 选择在真机运行时，需要用数据线连接手机
3. 如果小程序使用了微信云开发，转换为多端应用后，需要在 wx.cloud.init 中明确指定 appid 和 envid,还需要增加多端登录页（或者将云开发环境设置为未登陆模式），然后 app 才能正常使用。具体说明见：[多端应用使用微信云开发][16]

>

### 多端应用的绑定准备

多端应用接入小程序登陆/微信登陆等服务，还需要在[微信开放平台][8] 创建一个移动应用账号，微信开放平台中新创建的移动应用需要等1～7个工作日审核。

微信一共有下面三个管理平台：

* [微信公众平台][6]：公众号、小程序的注册和管理。
* [微信开发者平台][7]: 感觉是管理微信生态内的各种类型应用的，多端应用的有些操作需要在这里进行，
* [微信开放平台][8]: 接入微信相关的能力，比如微信登陆等，有一些能力比如微信支付需要是企业才能开通。

多端应用使用小程序登陆之前，需要绑定另外两个平台上的应用：

1. 和 [微信公众平台][6] 中的小程序绑定，小程序升级为多端应用时通常自动完成了绑定
2. 和 [微信开放平台][7] 中创建的移动应用绑定，移动应用需要在微信开放平台手动创建
3. 在 [微信开发者平台][8] 中查看以及管理多端应用，手动绑定移动应用

### 多端应用接入小程序登陆服务

完成前面的绑定后，按照文档操作： [快速接入小程序登录服务][9]
